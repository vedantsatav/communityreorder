...
pvector<ScoreT> Brandes(const Graph &g, SourcePicker<Graph> &sp,
                        NodeID num_iters, pvector<NodeID> &new_ids, bool isReordered = false) {
  Timer t;
  t.Start();
  ...
  for (NodeID iter=0; iter < num_iters; iter++) {
    ...
    cout << "old source: " << source << endl;

    if(isReordered)
    {
      source = new_ids[source];
    }
    cout << " new source: " << source << endl;
    t.Start();
    path_counts.fill(0);
...
{  
  ...
  t.Start();
  g = Builder::generateNeighbourMapping(g1, false, new_ids); //second arg = true to useOutDeg
  t.Stop();
}
else if (flag == 2)
{
  ...
  t.Start();
   g = Builder::generateDBGMapping(g1, false, new_ids); //second arg = true to useOutDeg
  t.Stop();
}
...
//for all sideways numbering
  ...
  auto BCBound =
    [&sp, &cli, &new_ids, &isReordered] (const Graph &g) { return Brandes(g, sp, cli.num_iters(), new_ids, isReordered); };
  SourcePicker<Graph> vsp(g, cli.start_vertex());
  auto VerifierBound = [&vsp, &cli] (const Graph &g,
                                     const pvector<ScoreT> &scores) {
    return BCVerifier(g, vsp, cli.num_iters(), scores);
  };
  BenchmarkKernel(cli, g, BCBound, PrintTopScores, VerifierBound, t.Seconds());
  return 0;
}

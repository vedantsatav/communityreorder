...
class CLBase {
 protected:
  ...
  char** argv_;
  std::string name_;
  std::string get_args_ = "f:g:hk:su:m:p:";
 ...
 public:
  CLBase(int argc, char** argv, std::string name = "") :
    ...
    AddHelpLine('p', "ordering", "Using which reordering");
    //AddHelpLine('x', "radius", "For BFS");
  ...
  void virtual HandleArg(signed char opt, char* opt_arg) {
    switch (opt) {
      ...
      case 'p': flag = atoi(opt_arg);                       break;
      //case 'x': radius = atoi(opt_arg);                       break;
    }
  }
  ...
  int flag_reordering() const{ return flag; }
  //int return_radius() const{return radius;}
};
...
// AddHelpLine('z', "ordering", "Using which reordering");
  }
  ...
  void HandleArg(signed char opt, char* opt_arg) override {
    ...
    // case 'z': flag = atoi(opt_arg);                       break;
 ...
 public:
  ...
  void HandleArg(signed char opt, char* opt_arg) override {
    switch (opt) {
      ...
      case 'z': flag_1 = atoi(opt_arg);                       break;
...
// int flag_reordering() const{ return flag_1; }
...
class CLPageRank : public CLApp {
 ...
 public:
  CLPageRank(int argc, char** argv, std::string name, double tolerance,
             int max_iters, int flag) :
    CLApp(argc, argv, name), max_iters_(max_iters), tolerance_(tolerance), flag_(flag) {
    get_args_ += "i:t:";
  ...
  void HandleArg(signed char opt, char* opt_arg) override {
    switch (opt) {
      ...
      case 't': tolerance_ = std::stod(opt_arg);            break;
          // case 'z': flag_ = atoi(opt_arg);                       break;
  ...
  double tolerance() const { return tolerance_; }
   // int flag_reordering() const{ return flag_; }
